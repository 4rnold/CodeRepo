...
public class AdminService {
	...
	public Admin adminLogin(Admin admin){
		//1.判断是否有此管理员
		Admin sysAdmin = adminDao.findByLoginname(admin.getLoginname());
		if(sysAdmin == null){
			throw new IllegalStateException("管理员不存在或者密码错误");
		}
		//2.校验密码
		boolean checkPassword = encoder.matches(admin.getPassword(),sysAdmin.getPassword());
		if(!checkPassword){
			throw new IllegalStateException("管理员不存在或者密码错误");
		}
		//3.返回

...
public abstract class BaseCaptcha extends Randoms {
    ...
    public String toBase64() {
        ...
        out(outputStream);
        return base64Encode() + Base64.getEncoder().encodeToString(outputStream.toByteArray());
    }
    ...
    protected char[] generateChars() {
        ...
        for (int i = 0; i < len; i++) {
            switch (charType) {
                case 2:
                    cs[i] = getChar(numMaxIndex);
                    break;
                case 3:
                    cs[i] = getChar(charMinIndex, charMaxIndex);
                    break;
                case 4:
                    cs[i] = getChar(upperMinIndex, upperMaxIndex);
                    break;
                case 5:
                    cs[i] = getChar(lowerMinIndex, lowerMaxIndex);
                    break;
                case 6:
                    cs[i] = getChar(upperMaxIndex);
                    break;
                default:
                    cs[i] = getChar();
            }

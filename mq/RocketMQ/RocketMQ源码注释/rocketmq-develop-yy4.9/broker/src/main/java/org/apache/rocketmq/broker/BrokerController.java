...
public class BrokerController {
    ...
    public BrokerController(final BrokerConfig brokerConfig, final NettyServerConfig nettyServerConfig,
        ...
        this.brokerConfig = brokerConfig;   //初始化配置对象
        this.nettyServerConfig = nettyServerConfig;
        this.nettyClientConfig = nettyClientConfig;
        this.messageStoreConfig = messageStoreConfig;
        //通过配置初始化管理器、任务线程类、监听器
    ...
    public boolean initialize() throws CloneNotSupportedException {
        //加载主题配置管理器、消费者偏移量管理器、订阅组管理器、消费者过滤管理器
        boolean result = this.topicConfigManager.load();
        result = result && this.consumerOffsetManager.load();
        result = result && this.subscriptionGroupManager.load();
        result = result && this.consumerFilterManager.load();
        if (result) {  //如果上面都初始化成功，构造消息存储
            try {
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);
                if (messageStoreConfig.isEnableDLegerCommitLog()) {  //如果开启了DlegerCL，构造对应的处理器
        ...
        result = result && this.messageStore.load();   //加载消息存储
        if (result) {   //如果消息存储加载成功，构造netty服务端进程
            ...
            this.registerProcessor();  //将处理器和线程池注册到netty的server上
            final long initialDelay = UtilAll.computeNextMorningTimeMillis() - System.currentTimeMillis();
            final long period = 1000 * 60 * 60 * 24;
            //启动各类周期任务（统计、消费者偏移量持久化、过滤器持久化、保护broker、打印水位线、分发落后CL、）
            ...
            //如果namesrv地址不为空，就更新namesrv地址列表，否则获取namesrv地址
            ...
            //如果开启了TLS，就周期监控tls文件，文件发生改变时重新加载
    ...
    private void initialTransaction() {
        ...
        if (null == this.transactionalMessageService) {  //如果反射初始化失败，就初始化一个默认事务消息服务
            this.transactionalMessageService = new TransactionalMessageServiceImpl(new TransactionalMessageBridge(this, this.getMessageStore()));
            log.warn("Load default transaction message hook service: {}", TransactionalMessageServiceImpl.class.getSimpleName());
        }
        //反射初始化一个事务消息检查监听器，如果反射初始化失败，就初始化一个默认的监听器
        ...
        this.transactionalMessageCheckService = new TransactionalMessageCheckService(this);  //初始化事务消息检查服务
    }

    //ok 初始化acl
    private void initialAcl() {
        if (!this.brokerConfig.isAclEnable()) {  //如果没开启acl，直接返回
        ...
        if (accessValidators == null || accessValidators.isEmpty()) {
            ...
            return;
        }  //遍历准入控制器，将其放入内存map中，并注册一个netty的钩子，在请求之前先进行准入验证
    ...
    //ok  注册处理器  将处理器和线程池注册到netty的server上
    ...
    public void protectBroker() {
        ...
        if (this.brokerConfig.isDisableConsumeIfConsumerReadSlowly()) {
            ...
            while (it.hasNext()) {  //就遍历统计信息，查看落后量，如果超过了阈值，就关闭该组的消费
    ...
    public void shutdown() {
        if (this.brokerStatsManager != null) {  //关闭统计管理器
            this.brokerStatsManager.shutdown();
        }
        if (this.clientHousekeepingService != null) {  //关闭各类服务线程类
            this.clientHousekeepingService.shutdown();
        }
        if (this.pullRequestHoldService != null) {
            this.pullRequestHoldService.shutdown();
        }
        if (this.remotingServer != null) {  //关闭netty服务端
        ...
        if (this.messageStore != null) {  //关闭消息存储
            this.messageStore.shutdown();
        }
        this.scheduledExecutorService.shutdown();  //关闭线程池
        try {
            this.scheduledExecutorService.awaitTermination(5000, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
        }
        this.unregisterBrokerAll();  //取消注册broker
        if (this.sendMessageExecutor != null) {  //关闭各类线程池
        ...
        this.consumerOffsetManager.persist();  //持久化消费偏移量

        if (this.filterServerManager != null) {  //关闭过滤服务管理器
            this.filterServerManager.shutdown();
        }

        if (this.brokerFastFailure != null) {  //关闭broker快速失败
            this.brokerFastFailure.shutdown();
        }

        if (this.consumerFilterManager != null) {  //关闭消费者过滤管理器
    ...
    public void start() throws Exception {
        ...
        //启动文件监听服务、外部访问API、拉请求控制服务、客户端保证服务
        if (this.fileWatchService != null) {
        ...
        //如果没开启DlegerCL，启动事务消息检查服务，周期向从节点同步全部数据，自己保证高可用
        if (!messageStoreConfig.isEnableDLegerCommitLog()) {
    ...
    public synchronized void registerBrokerAll(final boolean checkOrderConfig, boolean oneway, boolean forceRegister) {
        ...
        //如果强制注册或者判断需要注册，就执行注册方法，将主题配置注册到所有namesrv方法
    ...
    private void doRegisterBrokerAll(boolean checkOrderConfig, boolean oneway, TopicConfigSerializeWrapper topicConfigWrapper) {
        ...
        if (registerBrokerResultList.size() > 0) {
            ...
            if (registerBrokerResult != null) {  //如果注册结果中有高可用服务地址，就更新高可用master地址，更新从节点的master地址
                if (this.updateMasterHAServerAddrPeriodically && registerBrokerResult.getHaServerAddr() != null) {
                    this.messageStore.updateHaMasterAddress(registerBrokerResult.getHaServerAddr());
                }
                this.slaveSynchronize.setMasterAddr(registerBrokerResult.getMasterAddr());
                if (checkOrderConfig) {  //如果要检查有序配置，就更新有序配置
    ...
    private boolean needRegister(final String clusterName, final String brokerAddr, final String brokerName,
        ...
        //调用API判断是否有namesrv数据发生变化，只要有改变的，就返回true，代表需要注册
        TopicConfigSerializeWrapper topicConfigWrapper = this.getTopicConfigManager().buildTopicConfigSerializeWrapper();
    ...
    public void changeToSlave(int brokerId) {
        ...
        //change the role  将brokerId设置为1
        brokerConfig.setBrokerId(brokerId == 0 ? 1 : brokerId); //TO DO check
        messageStoreConfig.setBrokerRole(BrokerRole.SLAVE);  //角色设置为slave
        //handle the scheduled service  处理定时消息服务，如果是slave就关闭
        try {
            this.messageStore.handleScheduleMessageService(BrokerRole.SLAVE);
        } catch (Throwable t) {
            log.error("[MONITOR] handleScheduleMessageService failed when changing to slave", t);
        }
        //handle the transactional service  关闭事务消息检查服务
        try {
            this.shutdownProcessorByHa();
        } catch (Throwable t) {
            log.error("[MONITOR] shutdownProcessorByHa failed when changing to slave", t);
        }
        //handle the slave synchronise  处理从节点同步，周期向从节点同步全部数据
        handleSlaveSynchronize(BrokerRole.SLAVE);
        try {  //强制向所有namesrv注册信息
    ...
    public void changeToMaster(BrokerRole role) {
        if (role == BrokerRole.SLAVE) {  //如果传入角色是slave直接返回
            return;
        }
        log.info("Begin to change to master brokerName={}", brokerConfig.getBrokerName());
        //handle the slave synchronise  处理从节点同步，取消向本节点同步信息
        handleSlaveSynchronize(role);
        //handle the scheduled service   开启处理消息服务
        try {
            this.messageStore.handleScheduleMessageService(role);
        } catch (Throwable t) {
            log.error("[MONITOR] handleScheduleMessageService failed when changing to master", t);
        }
        //handle the transactional service  开启事务消息检查服务
        try {
        ...
        brokerConfig.setBrokerId(0); //TO DO check  id设置为0，角色设置为主

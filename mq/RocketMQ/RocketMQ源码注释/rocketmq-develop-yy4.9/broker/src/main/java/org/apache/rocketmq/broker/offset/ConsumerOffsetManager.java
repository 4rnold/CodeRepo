...
public class ConsumerOffsetManager extends ConfigManager {
    ...
    public Set<String> whichTopicByConsumer(final String group) {
        ...
        Iterator<Entry<String, ConcurrentMap<Integer, Long>>> it = this.offsetTable.entrySet().iterator();
        while (it.hasNext()) {  //遍历偏移量列表，获取所有的key（主题@组）
            Entry<String, ConcurrentMap<Integer, Long>> next = it.next();
            String topicAtGroup = next.getKey();
            //将key切分，找到和传入的组匹配的主题放入列表，最后返回
            String[] arrays = topicAtGroup.split(TOPIC_GROUP_SEPARATOR);
            if (arrays.length == 2) {
    ...
    private void commitOffset(final String clientHost, final String key, final int queueId, final long offset) {
        ConcurrentMap<Integer, Long> map = this.offsetTable.get(key); //根据key获取偏移量映射表
        if (null == map) {  //如果偏移量表为空，就初始化
            map = new ConcurrentHashMap<Integer, Long>(32);
            map.put(queueId, offset);
            this.offsetTable.put(key, map);
        } else {  //否则将队列id和偏移量放入表中，记录之前存储的偏移量
    ...
    public long queryOffset(final String group, final String topic, final int queueId) {
        // topic@group
        String key = topic + TOPIC_GROUP_SEPARATOR + group;  //根据主题和组构造key
        ConcurrentMap<Integer, Long> map = this.offsetTable.get(key); //获取指定key的偏移量映射表
        if (null != map) {   //从偏移量映射表中获取指定队列ID的偏移量返回，查不到就返回-1

...
public class EndTransactionProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request) throws RemotingCommandException {
        ...
        //如果broker类型是slave，返回从节点不支持响应
        if (BrokerRole.SLAVE == brokerController.getMessageStoreConfig().getBrokerRole()) {
            response.setCode(ResponseCode.SLAVE_NOT_AVAILABLE);
            LOGGER.warn("Message store is slave mode, so end transaction is forbidden. ");
            return response;
        }
        //对事务状态进行记录，如果不是提交和回滚直接返回空
        ...
        } else {  //如果开始事务检查为false，对事务状态进行记录，提交不记录，如果不是提交和回滚直接返回
            switch (requestHeader.getCommitOrRollback()) {
                case MessageSysFlag.TRANSACTION_NOT_TYPE: {
        ...
        //如果请求类型是提交，就调用事务消息服务提交消息
        if (MessageSysFlag.TRANSACTION_COMMIT_TYPE == requestHeader.getCommitOrRollback()) {
            result = this.brokerController.getTransactionalMessageService().commitMessage(requestHeader);
            if (result.getResponseCode() == ResponseCode.SUCCESS) { //如果提交成功了，就检查prepare消息
                RemotingCommand res = checkPrepareMessage(result.getPrepareMessage(), requestHeader);
                if (res.getCode() == ResponseCode.SUCCESS) {  //如果通过检查，就结束消息事务返回新消息
                    ...
                    RemotingCommand sendResult = sendFinalMessage(msgInner);  //发送这条新消息
                    if (sendResult.getCode() == ResponseCode.SUCCESS) { //如果消息发送成功，就删除prepare消息
            ...
            if (result.getResponseCode() == ResponseCode.SUCCESS) {  //如果回滚成功，就检查prepare消息，如果通过检查就删除prepare消息
                RemotingCommand res = checkPrepareMessage(result.getPrepareMessage(), requestHeader);

...
public class PullMessageProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        //获取请求中的消费者组的订阅组配置，如果配置为空，返回订阅组不存在
        ...
        if (hasSubscriptionFlag) {  //如果有订阅标识，就根据订阅的表达式类型构造订阅数据，出现异常就返回订阅解析失败
            try {
            ...
            //如果没有订阅标识，获取请求中指定消费者组的信息，如果信息为空，返回订阅不存在
            ...
            //从消费者组信息中获取指定主题的订阅数据，如果数据为空，返回订阅不存在
            ...
            //如果表达式类型不是tag，获取过滤数据为空就返回过滤数据不存在
            if (!ExpressionType.isTagType(subscriptionData.getExpressionType())) {
        ...
        if (getMessageResult != null) {
            ...
            //如果有消费消息钩子，就构造消费消息上下文，执行钩子before方法
            ...
            switch (response.getCode()) {
                ...
                case ResponseCode.PULL_RETRY_IMMEDIATELY:
                    break;
                    //如果状态是拉取偏移量移动，非slave就发送一个偏移量移动消息，否则响应码设置为立刻拉取retry
                case ResponseCode.PULL_OFFSET_MOVED:

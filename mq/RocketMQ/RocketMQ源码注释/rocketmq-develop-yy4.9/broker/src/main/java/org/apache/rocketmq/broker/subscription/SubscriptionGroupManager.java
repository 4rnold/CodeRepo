...
public class SubscriptionGroupManager extends ConfigManager {
    ...
    public SubscriptionGroupManager(BrokerController brokerController) {
        this.brokerController = brokerController;  //内部属性broker控制器赋值
        this.init();  //初始化
    }
    ...
    private void init() {
        ...
        {
            //构造CID_ONSAPI_PULL消费者组配置并放入订阅组列表
            SubscriptionGroupConfig subscriptionGroupConfig = new SubscriptionGroupConfig();
        ...
        {
            //构造CID_ONSAPI_PERMISSION消费者组配置并放入订阅组列表
            SubscriptionGroupConfig subscriptionGroupConfig = new SubscriptionGroupConfig();
        ...
        {
            //构造CID_ONSAPI_OWNER消费者组配置并放入订阅组列表
            SubscriptionGroupConfig subscriptionGroupConfig = new SubscriptionGroupConfig();
    ...
    public void disableConsume(final String groupName) {
        //根据组名从订阅组列表中获取订阅组配置，将配置信息设置为关闭消费状态，更新版本
    ...
    public SubscriptionGroupConfig findSubscriptionGroupConfig(final String group) {
        //根据组名获取订阅组配置，如果为空，判断如果开启了自动创建订阅组或者组名属于系统组，就新建一个订阅组配置并放入订阅组列表
        ...
        if (null == subscriptionGroupConfig) {
            if (brokerController.getBrokerConfig().isAutoCreateSubscriptionGroup() || MixAll.isSysConsumerGroup(group)) {
                ...
                this.dataVersion.nextVersion();  //然后更新版本并持久化
                this.persist();
            }
        }
        return subscriptionGroupConfig;  //返回指定组名的订阅组配置
    }

...
public class TopicConfigManager extends ConfigManager {
    ...
    public TopicConfigManager(BrokerController brokerController) {
        this.brokerController = brokerController;  //内部broker控制器赋值
        {
            //构造SELF_TEST_TOPIC的主题配置
        ...
        {
            //如果开启了自动创建主题，就构造AUTO_CREATE_TOPIC_KEY_TOPIC配置
        ...
        {
            //构造RMQ_SYS_OFFSET_MOVED_EVENT配置
    ...
    public TopicConfig createTopicInSendMessageMethod(final String topic, final String defaultTopic,
            ...
            if (this.topicConfigTableLock.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {  //上锁，获取指定主题的配置，如果不为空直接返回
                    ...
                    if (defaultTopicConfig != null) {
                        //如果默认主题是TBW102并且没开启自动创建主题，就设置主题配置为读写
                        if (defaultTopic.equals(TopicValidator.AUTO_CREATE_TOPIC_KEY_TOPIC)) {
                            if (!this.brokerController.getBrokerConfig().isAutoCreateTopicEnable()) {
                                defaultTopicConfig.setPerm(PermName.PERM_READ | PermName.PERM_WRITE);
                            }
                        }
                        //如果默认主题配配置是可继承的，就用主题新建一个主题配置并用默认主题配置的属性设置其属性
                    ...
                    //如果主题配置不为空，就把主题配置加入主题配置列表，标记为新创建，更新版本并持久化
                    if (topicConfig != null) {
        ...
        if (createNew) {  //如果新创建了主题配置，就向所有namesrv注册broker
    ...
    public TopicConfig createTopicInSendMessageBackMethod(final String topic, final int clientDefaultTopicQueueNums,
        final int perm, final int topicSysFlag) {  //获取指定主题的配置，如果不为空直接返回
        ...
        try {
            if (this.topicConfigTableLock.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {  //否则上锁再获取一次主题配置，如果不为空直接返回，如果为空，使用主题新建一个主题配置，标记为新创建并持久化
                    topicConfig = this.topicConfigTable.get(topic);
                    if (topicConfig != null)
                        return topicConfig;
                    topicConfig = new TopicConfig(topic);
        ...
        if (createNew) {  //如果新建了主题配置，就向所有namesrv注册broker
            this.brokerController.registerBrokerAll(false, true, true);
        }
        return topicConfig;
    }

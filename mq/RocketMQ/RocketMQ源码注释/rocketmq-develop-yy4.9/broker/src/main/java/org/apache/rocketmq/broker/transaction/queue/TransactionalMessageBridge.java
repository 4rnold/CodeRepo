...
public class TransactionalMessageBridge {
    ...
    public long fetchConsumeOffset(MessageQueue mq) {
        //使用偏移量管理器查询指定mq的主题和id在事务系统组中的偏移量，如果偏移量为-1，就获取MQ中最小偏移量
    ...
    public Set<MessageQueue> fetchMessageQueues(String topic) {
        ...
        TopicConfig topicConfig = selectTopicConfig(topic);  //获取指定主题的配置
        if (topicConfig != null && topicConfig.getReadQueueNums() > 0) {
            //获取配置中的读队列数，构造对应个数的mq集合返回
    ...
    private PullResult getMessage(String group, String topic, int queueId, long offset, int nums, SubscriptionData sub) {
        //利用消息存储对象从指定组主题和队列id中的CQ的指定偏移量，最多查询max数量的消息
        ...
        if (getMessageResult != null) {
            ...
            switch (getMessageResult.getStatus()) {
                //根据查询结果状态设置拉取状态，如果状态是found，就解码出消息列表，更新broker统计管理器相关数据，其余状态都日志记录错误信息
                case FOUND:
    ...
    public boolean putOpMessage(MessageExt messageExt, String opType) {
        //通过传入的消息体和操作类型构造一个MQ，如果操作类型是删除标签，就在事务操作中添加删除标签，否则直接返回
    ...
    private void writeOp(Message message, MessageQueue mq) {
        ...
        //尝试从op队列表获取传入的半消息MQ对应的op队列
        if (opQueueMap.containsKey(mq)) {
            opQueue = opQueueMap.get(mq);
        } else {
            //获取失败就根据传入的半消息队列生成一个op队列并放入op队列表

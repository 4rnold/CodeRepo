...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private final ConcurrentMap<String/* topic */, TopicPublishInfo> topicPublishInfoTable =
        new ConcurrentHashMap<String, TopicPublishInfo>();  //主题发布信息表，key是主题，value是主题发布信息
    ...
    //ok  初始化事务环境，其实就是创建检查线程池
    public void initTransactionEnv() {
        TransactionMQProducer producer = (TransactionMQProducer) this.defaultMQProducer;
        //如果默认生产者的线程池不为空，就将其设置为检查线程池
        if (producer.getExecutorService() != null) {
            this.checkExecutor = producer.getExecutorService();
        } else {
            //否则新建一个检查线程池，容量1，1，keepalive是60s，队列容量2000
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            ...
            case CREATE_JUST:
                ...
                this.checkConfig();
                //如果生产者的组不是CLIENT_INNER_PRODUCER，就改实例id为pid
                ...
                if (!registerOK) {  //如果注册失败，就抛异常，改服务状态为刚创建（默认）
                    this.serviceState = ServiceState.CREATE_JUST;
                    throw new MQClientException("The producer group[" + this.defaultMQProducer.getProducerGroup()
                        + "] has been created before, specify another name please." + FAQUrl.suggestTodo(FAQUrl.GROUP_NAME_DUPLICATE_URL),
                        null);
                }
                //主题发送信息列表放入生产者的主题，新建主题发送信息
                this.topicPublishInfoTable.put(this.defaultMQProducer.getCreateTopicKey(), new TopicPublishInfo());
                //如果入参为true，就开启客户端（开启底层netty客户端，定时获取namesrv地址，开启拉消息线程、负载线程、生产者线程）
                if (startFactory) {
        ...
        this.mQClientFactory.sendHeartbeatToAllBrokerWithLock();   //向所有broker发心跳
        RequestFutureHolder.getInstance().startScheduledTask(this);  //开始定时任务，定时删除过期请求
    }
    ...
    public void shutdown(final boolean shutdownFactory) {
        switch (this.serviceState) {
            case CREATE_JUST:
                break;
            case RUNNING:
                //如果服务状态是running，就用客户端取消注册生产者，关闭异步分发线程池
    ...
    public void checkTransactionState(final String addr, final MessageExt msg,
        ...
        Runnable request = new Runnable() {
            ...
            @Override
            public void run() {
                //获取事务监听器，然后获取消息的事务状态，调用方法处理事务状态
            ...
            private void processTransactionState(
                ...
                //创建结束事务请求header并赋予属性，根据事务状态设置提交还是回滚
                ...
                try {
                    //根据header通过客户端调用单向结束事务，在方法内创建了一个结束事务Command然后调用remoting模块发送单向消息
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            //设置尝试总次数，如果是同步，就是1+重试次数，异步和单向都是1
            ...
            for (; times < timesTotal; times++) {
                //获取上次发送的broker名，第一轮循环这里为null
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                //根据主题和broker获取一个消息队列，具体逻辑在latency包的MQFaultStrategy中
                MessageQueue mqSelected = this.selectOneMessageQueue(topicPublishInfo, lastBrokerName);
                if (mqSelected != null) {
                    mq = mqSelected;
                    //brokersSent数组为了记录尝试向哪些broker发送，后面日志记录用
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                SendResult sendResult = null;
                switch (communicationMode) {
                    //根据模式，调用MQClientAPIImpl的sendMessage方法发送消息，sendMessage会根据传入的请求头构建Command，
                    // 根据对应模式调用client的同步/异步/单向invoke
                    case ASYNC:
    ...
    private SendResult sendSelectImpl(Message msg, MessageQueueSelector selector, Object arg,
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            try {
                ...
                //和不带选择器的区别在这里
                // 1、选择消息队列方式不同：一个是选择器选消息队列，一个是根据主题发布信息和上一次broker选消息队列
                // 2、选择器方式没有重试。
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        // ignore DelayTimeLevel parameter 忽略延迟时间级别
    ...
    public void endTransaction(
        ...
        //APIImpl根据header调用单向结束事务方法，在这里构建了Command，然后调用单向invoke

...
public abstract class ComparisonExpression extends BinaryExpression implements BooleanExpression {
    ...
    //创建一个between的布尔表达式  value between left和right
    public static BooleanExpression createBetween(Expression value, Expression left, Expression right) {
        // check left和right都必须是常数表达式
        ...
        //用value和left创建≥表达式，用value和right创建≤表达式，用这两个表达式创建一个&&表达式
        //相当于value≥left && value≤right
        return LogicExpression.createAND(createGreaterThanEqual(value, left), createLessThanEqual(value, right));
    }

    //创建value not between left和right
    ...
    public static BooleanExpression createGreaterThanEqual(final Expression left, final Expression right) {
        //检查left和right表达式是否支持大于小于
        checkLessThanOperand(left);
        checkLessThanOperand(right);
        //创建一个比较表达式，值正为true，表达式符号是大于等于
        return new ComparisonExpression(left, right) {
            protected boolean asBoolean(int answer) {
                return answer >= 0;
            }
    ...
    //ok  比较左值和右值，如果类型不一致就尝试转换成支持比较的类型再比较
    @SuppressWarnings({"rawtypes", "unchecked"})

...
public class GetMessageResult {
    ...
    private final List<SelectMappedBufferResult> messageMapedList;  //匹配的消息列表
    private final List<ByteBuffer> messageBufferList;  //消息buf列表
    private GetMessageStatus status;   //获取消息状态
    private long nextBeginOffset;   //下一个开始偏移量
    private long minOffset;   //最小偏移量
    private long maxOffset;   //最大偏移量
    private int bufferTotalSize = 0;  //buffer总大小
    private boolean suggestPullingFromSlave = false;   //是否建议从节点拉取
    private int msgCount4Commercial = 0;   //贸易消息数
